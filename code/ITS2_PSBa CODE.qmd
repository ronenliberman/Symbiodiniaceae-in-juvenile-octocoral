---
title: "Quarto file Code for Making figures 1-3"
...
require("knitr")
opts_knit$set(root.dir = "~/PhD/Konstanz Collaboration/Genomic data/symportal/R codes and scripts/Code in one place/")
```
...
pacman::p_load("ggplot2", "dplyr","plyr","ggsci", "reshape2", "MCMC.OTU", "vegan", "edgeR", "tidyverse",
               "gt", "vegan", "pairwiseAdonis", "RColorBrewer", "Redmonder","ggpubr", "rmarkdown")
...
profRAW = read.delim("BarPlot/203_20220411_03_DBV_20220412T020330.profiles.absolute.abund_only.txt", header = TRUE, check.names = FALSE)
head(profRAW)

#View(profRAW)
MetaData = read.csv("BarPlot/Metadata1.csv")  ### Loading metadata 
head(MetaData)
...
lab= read.csv("BarPlot/Our Profiles.csv")
...
new_df <- read.csv("BarPlot/factors_added_changed.csv") ## another metadata file to create the reciprocal experminet plot 
head(new_df)
...
```{r 1-week, echo=TRUE, warning=FALSE}
#time point 1
...
```{r 2 Months, echo=TRUE, warning=FALSE}
...
```{r 5 Months, echo=TRUE, warning=FALSE}
...
```{r 7 Months, echo=TRUE, warning=FALSE}
seven <- Sum_this %>% 
  filter(time == "7")
view(seven)
...
```{r 11 months, echo=TRUE, warning=FALSE}
eleven <- Sum_this %>% 
  filter(time == "11")
view(eleven)
...
```{r Adults, warning=FALSE,echo=TRUE}
...
adult <- read.csv("BarPlot/all_adults_purged.csv")
#select for R. fulvum data
ad <- adult %>% 
  filter(Species=="R. fulvum") 
head(ad)
ex.dat= read.csv("BarPlot/Adults_real_type profile.csv")
...
### *STEP 7* e all the plots into oneCombin

```{r Put into one, echo=TRUE, warning=FALSE}
...
its2_parents = read.delim(
  "pcoA/177_20210916_all_03_DBV_20210917T005252.seqs.absolute.abund_only.txt", header = TRUE, check.names = FALSE)

#Juvenile PCOA data
its2_poly=read.delim("pcoA/203_20220411_03_DBV_20220412T020330.seqs.absolute.abund_only.txt", header = TRUE, check.names = FALSE) 
...
meta <- read.csv("pcoA/Metadata_Adult_And_Pol.csv") #load metadata for both juvenile and adults
...
**Symbiodinium** :
```{r, echo=TRUE}
dt <- read.csv("pcoA/20220412T020330_braycurtis_samples_PCoA_coords_A_sqrt.csv",
      header = TRUE)

MetaData = read.csv("pcoA/Metadata.csv",header = TRUE)
...
p2 <- pco_two_fac +#stat_ellipse(geom="polygon", aes(fill = source.depth), 
...
dt <- read.csv("pcoA/20220412T020330_braycurtis_samples_PCoA_coords_D_sqrt.csv",header = TRUE)
...
iq= read.iqtree("TREE_psbA/raw_data/all.psba.no27.wc3gulf.aligned.fasta.contree")
#iq= read.iqtree("all.psba.no27.wc3gulf.aligned.fasta.contree")  

tib_q <- as_tibble(iq)   ## look at the data in a table format  

sample_data_1 <- import("TREE_psbA/raw_data/phylo_dat_for_iqtree_new.csv") # import the metadata 

sample_data_2 <- sample_data_1 %>% 
  rename(c("Source" = "group"))
#view(sample_data_2)

q <- ggtree(iq, layout="circular")  ## basic tree 
#q 

dat_q_no_metadat <- as.treedata(q)

#tib_q_no_metadat <- as_tibble(dat_q_no_metadat)
#view(tib_q_no_metadat)

q1 <- q %<+% sample_data_2 + geom_rootpoint(color="black", size=.5) + hexpand(.5, direction = -1)  ##expand tree              ## add metadata
q1

q2 <- q1 +                      ### show a tree with only specific names on the tree - In this case, My samples ,
                                ##  check the data table, they they all start with _mo
...
#use this to add symbols instead of true bootstrap numbers geom_point2(aes(label =SH_aLRT, subset= SH_aLRT>80))
#use this to try and avoid the overlapping text. no fully developed . geom_label_repel(aes(label=SH_aLRT, fill= SH_aLRT, size = 1))
q2


q3 <- q2+                                    ### add the information of the origin of the data 
  geom_tippoint(
    mapping = aes(colour= Sample_origin),          # tip color by continent. You may change shape adding "shape = "
    size = 2.5,alpha=0.35 )+                             # define the size of the point at the tip
  scale_colour_brewer(
...
#ggsave(plot=q4, file="circ_tree_with_bootstrap_new.jpeg", dpi= 600,width =26 , height = 25,
 #      units = "cm")  
```
